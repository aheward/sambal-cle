...
class Sections < SectionsBase
  ...
  action(:edit) { |title, b| b.sections_table.row(:text=>/#{Regexp.escape(title)}/).link(:text=>/Edit/).click }

  action(:assign_tas) { |title, b| b.sections_table.row(:text=>/#{Regexp.escape(title)}/).link(:text=>/Assign TAs/).click }

  action(:assign_students) { |title, b| b.sections_table.row(:text=>/#{Regexp.escape(title)}/).link(:text=>/Assign Students/).click }

  action(:check) { |title, b| b.sections_table.row(:text=>/#{Regexp.escape(title)}/).checkbox(:name=>/remove/).set }

  def section_names
    names = []
    sections_table.rows.each do |row|
  ...
  # Section.
  action(:tas_for) { |title, b| b.sections_table.row(:text=>/#{Regexp.escape(title)}/)[1].text }

  #
  action(:days_for) { |title, b| b.sections_table.row(:text=>/#{Regexp.escape(title)}/)[2].text }

  #
  action(:time_for) { |title, b| b.sections_table.row(:text=>/#{Regexp.escape(title)}/)[3].text }

  #
  action(:location_for) { |title, b| b.sections_table.row(:text=>/#{Regexp.escape(title)}/)[4].text }

  #
  action(:current_size_for) { |title, b| b.sections_table.row(:text=>/#{Regexp.escape(title)}/)[5].text }

  #
  action(:availability_for) { |title, b| b.sections_table.row(:text=>/#{Regexp.escape(title)}/)[6].text }
  ...
  element(:sections_table) { |b| b.frm.table(:class=>/listHier/) }
