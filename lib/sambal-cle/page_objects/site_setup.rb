...
class SiteEditor < SiteSetupBase
  ...
  link 'Edit Site Information'
  link 'Edit Tools'
  link 'Add Participants'
  link 'Edit Class Roster(s)'
  link 'Manage Groups'
  link 'Link to Parent Site'
  link 'External Tools'
  link 'Manage Access'
  link 'Duplicate Site'
  link 'Import from Site'
  link 'Import from Archive File'
...
class SiteSetupChooseRole < SiteSetupBase
...
class SiteSetupParticipantConfirm < SiteSetupBase
  ...
  action(:id) { |name, b| b.participant_table.row(:text=>/#{Regexp.escape(name)}/)[1].text }
  
  # Returns the value of the Role field for the specified name.
  action(:role) { |name, b| b.participant_table.row(:text=>/#{Regexp.escape(name)}/)[2].text }

  # ==========
  private
  # ==========
  
  element(:participant_table) { |b| b.frm.table(:class=>'listHier') } 
...
class CourseSiteInfo < SiteSetupBase
...
# 
class PortfolioSiteTools < SiteSetupBase
...
# 
class PortfolioConfigureToolOptions < SiteSetupBase
